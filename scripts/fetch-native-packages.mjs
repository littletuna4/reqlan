 ...
 * GitHub Actions (deploy-npm.yml, analytical/v* from a changeset) builds and
 * publishes those packages; this script only consumes the registry.
 *
 * Usage:
...
const retryDelayMs = Number.parseInt(parseArg(argv, '--retry-delay-ms') ?? '15000', 10);
...
while (true) {
    ...
    if (!version) {
        ...
        if (missing.length === 0) {
            console.log(`fetched ${targets.length} platform package(s) at ${version}`);
            process.exit(0);
        }
        console.error(`Missing ${missing.length} package(s) at ${version}: ${missing.join(', ')}`);
        if (allowMissing) {
    ...
    attempt += 1;
    console.log(`retry ${attempt}/${retries} in ${retryDelayMs}ms (waiting for npm publish)`);
