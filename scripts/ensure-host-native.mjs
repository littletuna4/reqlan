 ...
 *   2. Build and stage with cargo (`stage-host-native.mjs`, forced).
 *
 * Usage:
 *   node scripts/ensure-host-native.mjs
 *   node scripts/ensure-host-native.mjs --no-build   # existing binary only
 *
...
if (noBuild) {
  throw new Error(
    `Host native optionalDependency is missing for ${target.napiSuffix}. ` +
      `Stage the matrix binary (prepare-native-packages.mjs --binary-dir) or omit --no-build so cargo can build reqlan-napi.`,
  );
}

console.log("building reqlan-napi for the workspace package");
const ok = run(
  process.execPath,
  [path.join(root, "scripts/stage-host-native.mjs")],
  {
    ...process.env,
...
if (!ok || !binaryExists(packageBinaryPath())) {
  throw new Error(
    `Failed to populate ${packageBinaryPath()}. Install Rust and retry.`,
  );
}
