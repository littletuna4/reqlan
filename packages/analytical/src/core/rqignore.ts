...
import { APPLICATION_MEMORY_DIR, RQIGNORE_FILENAME } from './application-memory.js';
...
function createFilter(ig: Ignore): RqIgnoreFilter {
    return {
        ignores(relativePath: string, isDirectory = false): boolean {
            ...
            const candidate = isDirectory && !normalized.endsWith('/')
                ? `${normalized}/`
                : normalized;
 ...
 */
export function createRqIgnoreFilter(extraPatterns?: string | readonly string[]): RqIgnoreFilter {
    const ig = ignore();
