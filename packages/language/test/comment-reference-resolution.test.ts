...
describe('comment reference resolution', () => {
    ...
    test('shares one presentation so a matching idea clears the underline and adds a link', () => {
        ...
        const resolved = presentCommentReferences(text, '/src', host, posix.resolve);
...
describe('comment reference resolution in .rq documents', () => {
    ...
    test('publishes a missing-idea diagnostic and no document link until the idea exists', async () => {
        ...
        `);
        await services.shared.workspace.DocumentBuilder.build([target, source], { validation: true });
        ...
        const links = await services.Reqlan.lsp.DocumentLinkProvider?.getDocumentLinks(source, {
            textDocument: { uri: source.textDocument.uri }
        });
        expect(links?.some(link => link.target?.includes('syntax.rq'))).toBeFalsy();
    });
    ...
    test('clears the underline and creates a link after a matching idea is added', async () => {
        ...
        `);
        await services.shared.workspace.DocumentBuilder.build([target, source], { validation: true });
        expect(source.diagnostics?.some(diagnostic => diagnostic.code === COMMENT_REFERENCE_MISSING_IDEA)).toBe(true);

        await clearDocuments(services.shared, [target]);
        const updatedTarget = await parseAt(targetPath, s`
            present_idea {
                body
            }
            elevator_pitch {
        ...
        `);
        await services.shared.workspace.DocumentBuilder.build([updatedTarget, source], { validation: true });
    ...
    test('relinks documents that still show a missing comment-reference idea', () => {
        ...
        expect(shouldRelinkCommentReferences(
            {
                textDocument: { getText: () => 'host {}\n' },
                diagnostics: [{ code: COMMENT_REFERENCE_MISSING_IDEA }]
            } as unknown as LangiumDocument,
...
describe('comment reference resolution after workspace updates', () => {
    ...
    test('revalidates comment-reference underlines when a matching idea is created on disk', async () => {
        const services = createReqlanServices(NodeFileSystem);
        expect(services.shared.workspace.DocumentBuilder).toBeInstanceOf(ReqlanDocumentBuilder);
        ...
        const target = await services.shared.workspace.LangiumDocumentFactory.fromUri(targetUri) as LangiumDocument<Model>;
        const source = await services.shared.workspace.LangiumDocumentFactory.fromUri(sourceUri) as LangiumDocument<Model>;
        ...
        expect(source.diagnostics?.some(diagnostic => diagnostic.code === COMMENT_REFERENCE_MISSING_IDEA)).toBe(false);
        const presented = presentCommentReferencesForDocument(
            source,
            services.shared.workspace.LangiumDocuments,
            services.shared.workspace.FileSystemProvider
