...
/** Padding (px) around the bounding box when fitting the graph to the viewport. */
export const GRAPH_VIEWPORT_PADDING = 36;
...
/**
 * Fit all visible graph elements into the cytoscape viewport and center the camera.
 * Does not change node positions or restart layout/physics.
 */
export function reframeGraphToViewport(
    cy: cytoscape.Core,
    options: ReframeViewportOptions = {}
): void {
    const padding = options.padding ?? GRAPH_VIEWPORT_PADDING;
    const animate = options.animate ?? false;
    const duration = options.duration ?? GRAPH_REFRAME_ANIMATION_MS;
...
export interface GraphCyControllerOptions {
    ...
    /** summary: tap selects, dbltap opens. sidebar: tap opens, dbltap focuses. */
    interactionMode?: 'summary' | 'sidebar';
    onRendered?: () => void;
...
export class GraphCyController {
    ...
    /** Fit the current graph into the viewport and center the camera. */
    reframeToViewport(options: ReframeViewportOptions = {}): void {
        if (!this.cy) {
            return;
        }
        const animate = options.animate ?? this.hasReframed;
        reframeGraphToViewport(this.cy, {
            padding: options.padding ?? GRAPH_VIEWPORT_PADDING,
            animate,
            duration: options.duration
        });
        this.fittedNodeSetKey = this.lastSyncedNodeSetKey;
        this.hasReframed = true;
    }

    /** @deprecated Use reframeToViewport — kept for existing callers. */
    resetView(): void {
        this.reframeToViewport();
    }
    ...
    private bindInteractions(instance: cytoscape.Core): void {
        this.unbindInteractions = bindGraphInteractions(instance, {
            onNodeTap: (nodeId) => {
                ...
                if (this.options.interactionMode === 'sidebar') {
    ...
    /** Reframe when the synced node set changed; first fit is instant, later fits animate. */
    private maybeAutoReframeAfterSync(): void {
        if (!this.cy || this.cy.elements().length === 0) {
            return;
        }
        if (this.lastSyncedNodeSetKey === this.fittedNodeSetKey) {
            return;
        }
        this.reframeToViewport({ animate: this.hasReframed });
    }
