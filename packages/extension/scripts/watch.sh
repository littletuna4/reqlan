...
set -euo pipefail
cd "$(dirname "$0")/.."
exec concurrently -n webviews,activity,onboarding,export,tsc,esbuild,syntax -c magenta,green,red,white,blue,yellow,cyan \
    ...
    "vite build --config webviews/export-form/vite.config.ts --watch" \
