 ...
 * and add_to_chat in ["../../../../reqlan rq/extension/agent/features-ai.rq"]
 */
...
export function registerAiCommandsModule(
  ...
  context.subscriptions.push(
    ...
    vscode.commands.registerCommand(
      "reqlan.addIdeaToChat",
      async (args?: AddToChatArgs) => {
    ...
    vscode.commands.registerCommand("reqlan.addIdeasetToNewChat", async () => {
      await vscode.commands.executeCommand("reqlan.addIdeasetToChat", {
        newChat: true,
      });
    }),

    vscode.commands.registerCommand(
      "reqlan.addFileToChat",
      async (args?: AddToChatArgs) => {
    ...
    vscode.commands.registerCommand("reqlan.addFileToNewChat", async () => {
      await vscode.commands.executeCommand("reqlan.addFileToChat", {
        newChat: true,
      });
    }),
      ...
      async (promptName?: string) => {
        const selected =
          ...
          (await vscode.window.showQuickPick(
            [
              {
                label: "rq-build-requirement",
                description: "Create a requirement from intent",
              },
              {
                label: "rq-add-to-context",
                description: "Attach requirement context to chat",
              },
              {
                label: "rq-write-plan",
                description: "Draft an @plan attribute",
              },
              {
                label: "rq-search-requirements",
                description: "Search the requirement graph",
              },
              {
                label: "rq-file-context",
                description: "Summarise requirements for a file",
              },
            ],
...
async function maybePromptCursorSkillsInstall(
  context: vscode.ExtensionContext,
): Promise<void> {
...
async function resolveIdeaForChat(
  submodule: AnalyticalSubmodule,
): Promise<IdeaSummary | undefined> {
  await ensureIndex(submodule);
  const focus = getEditorFocus();
  if (focus) {
    const ideas = await submodule.index.indexStore.listIdeasInFileWithRanges(
      focus.fileUri,
    );
    ...
    const atCursor =
      findIdeaAtLine(ideas, focus.line) ??
      (await submodule.index.indexStore.getIdeaAtLine(
        focus.fileUri,
        focus.line,
      ));
    if (atCursor) {
    ...
    if (ideas.length > 0) {
      const nearCursor = [...ideas].sort(
        (a, b) =>
          Math.abs(a.lineStart - focus.line) -
          Math.abs(b.lineStart - focus.line),
      );
...
async function resolveIdeasetForChat(
  submodule: AnalyticalSubmodule,
): Promise<IdeaSummary | undefined> {
  await ensureIndex(submodule);
  const focus = getEditorFocus();
  ...
  if (focus) {
    const ideasets =
      await submodule.index.indexStore.listIdeasetsInFileWithRanges(
        focus.fileUri,
      );
    preferred = ideasets;
    if (focus.selectionRange) {
      ...
      if (inSelection.length > 1) {
        return pickFromIdeas(
          inSelection,
          "Select an ideaset from the selection",
        );
      }
    }
    const atCursor =
      findIdeaAtLine(ideasets, focus.line) ??
      (await submodule.index.indexStore.getIdeasetAtLine(
        focus.fileUri,
        focus.line,
      ));
    if (atCursor) {
  ...
  if (editor && vscode.workspace.getWorkspaceFolder(editor.document.uri)) {
    ...
    if (
      ...
      ideas = [
        ...
        (idea, idx, list) =>
          list.findIndex((entry) => entry.id === idea.id) === idx,
      );
    }
  }
  if (ideas.length === 0) {
...
async function pickIdeaset(
  ...
  await ensureIndex(submodule);
  const all = (await submodule.index.indexStore.listAllIdeas()).filter(
    (idea) => idea.kind === "ideaset",
  );
  ...
  if (ideasets.length === 0) {
    void vscode.window.showInformationMessage(
      "No ideasets found in the workspace.",
    );
    return undefined;
  }
...
async function pickRqFile(): Promise<
  { fileUri: string; relativePath: string } | undefined
> {
  ...
  if (files.length === 0) {
    void vscode.window.showInformationMessage(
      "No .rq files found in the workspace.",
    );
    return undefined;
  }
  ...
  const entries = files
    ...
    .filter(
      (entry) => !needle || entry.relativePath.toLowerCase().includes(needle),
    )
    .slice(0, 50);
...
async function formatIdeasetContext(
  ...
  const memberLines = members
    ...
    .map(
      (member) =>
        `- ${member.name} (${vscode.workspace.asRelativePath(member.fileUri)})`,
    )
    .join("\n");
  return `${base}\nKind: ideaset\nMembers:\n${memberLines}`;
}
...
async function formatFileContext(
  ...
  const ideas = (
    await submodule.index.indexStore.getIdeasInFile(fileUri)
  ).filter((idea) => idea.kind !== "ideaset");
...
async function sendIdeaToChat(
  ...
  const opened = await openChatWithText(
    `#requirement ${idea.name}\n\n${contextText}`,
    {
      isPartialQuery: true,
      target,
    },
...
async function sendFileToChat(
  ...
  const contextText = await formatFileContext(
    submodule,
    file.fileUri,
    file.relativePath,
  );
