 ...
 * per ["../../../../reqlan rq/extension/workspace-summary/ideas_summary/webview.rq".ideas_summary]
 * per ["../../../../reqlan rq/extension/workspace-summary/ideas_summary/webview.rq".ontology_aligned_tabs]
...
export class IdeasSummaryPanel {
  ...
  static show(
    ...
    IdeasSummaryPanel.current = new IdeasSummaryPanel(
      context,
      submodule,
  ...
  private referencesQuery: ReferencesTableQuery = {
    ...DEFAULT_REFERENCES_QUERY,
  };
  private attributesQuery: AttributesTableQuery = {
  ...
  private constructor(
    ...
    this.panel.webview.html = getIdeasSummaryHtml(
      this.panel.webview,
      this.context.extensionUri,
    );
  ...
  applyNavigateIntent(intent: IdeasSummaryNavigateIntent): void {
    ...
    if (
      intent.centerId ||
      intent.includeIndirect !== undefined ||
      intent.pathFilter
    ) {
      this.graphQuery = {
        ...this.graphQuery,
        ...
        includeIndirect:
          intent.includeIndirect ?? this.graphQuery.includeIndirect,
  ...
  private readGraphUiState(): GraphUiPersistedState {
    const key = graphUiWorkspaceStateKey(
      this.submodule.index.getActiveBaseId(),
    );
    const raw =
  ...
  private postTableUiState(): void {
    ...
    if (state.references.groupBy) {
      this.referencesQuery = {
        ...this.referencesQuery,
        groupBy: state.references.groupBy,
      };
    }
  ...
  private async sendIdeasPage(): Promise<void> {
    ...
    const rows = (await store.listIdeasPage(resolvedQuery)).map((row) => ({
      ...row,
  ...
  private async sendIdeasetsPage(): Promise<void> {
    ...
    const safePage = clampPage(query.page, total, query.pageSize);
    const resolvedQuery = { ...query, page: safePage };
    const rows = (await store.listIdeasetsPage(resolvedQuery)).map((row) => ({
      ...row,
      path: this.toDisplayPath(row.path),
    }));
    this.post({
  ...
  private async sendReferencesPage(): Promise<void> {
    ...
    const safePage = clampPage(query.page, total, query.pageSize);
    const resolvedQuery = { ...query, page: safePage };
    const rows = (await store.listReferencesPage(resolvedQuery)).map((row) => ({
      ...row,
  ...
  private async sendTimelinePage(): Promise<void> {
    ...
    if (this.submodule.index.isReady) {
      const rows =
        await this.submodule.index.indexStore.listRecentGitIdeaEvents(100);
      gitEvents = rows
        .map((row) => {
  ...
  private async buildIdeaIndexTimelineEvents(): Promise<
    Array<{
      id: string;
      source: "index";
      at: number;
  ...
  > {
    const updates =
      this.submodule.index.getStatusSnapshot().recentDocumentUpdates ?? [];
    const events: Array<{
      id: string;
      source: "index";
      at: number;
    ...
    const ideaIds = updates.flatMap((update) =>
      (update.ideas ?? []).map((idea) => idea.id),
    );
    const byId = new Map<
      string,
      Awaited<ReturnType<typeof this.submodule.index.indexStore.getIdea>>
    >();
    if (this.submodule.index.isReady && ideaIds.length > 0) {
      const ideas =
        await this.submodule.index.indexStore.getIdeasByIds(ideaIds);
...
function normalizeGraphQuery(query: GraphViewQuery): GraphViewQuery {
  const truncationBasis =
    query.truncationBasis === "git-modified" ||
    query.truncationBasis === "git-created"
  ...
  return {
    ...
    maxNodes: Math.min(
      Math.max(1, query.maxNodes ?? GRAPH_MAX_NODES),
      GRAPH_NODES_HARD_CAP,
    ),
    truncationBasis,
  };
...
function normalizeGraphFilterList(
...
async function defaultGraphCenterId(
  submodule: AnalyticalSubmodule,
...
function normalizeReferencesQuery(
...
function normalizeAttributesQuery(
